...
module axi4_tb;
    ...
    initial begin
        ...
        if ($signed(rd_result) == 32'sd11749) begin
            $display("  [PASS]");
            pass_cnt = pass_cnt + 1;
        end else begin
            $display("  [FAIL] expected 11749");
            fail_cnt = fail_cnt + 1;
        end
        ...
        // Objects 0-3: obj_mem0, low scores
        for (k = 0; k < 4; k = k + 1) begin
            for (i = 0; i < EMB_DIM; i = i + 1)
                burst_buf[i] = {24'h0, obj_mem0[i]};
            programme_object(k, 32'd8000, 32'd1000, 32'h0);
        end

        // Objects 4-7: obj_mem1, low scores
        ...
        // Object 5: highest metadata → expected winner
        axi_write(32'h0200 + 5*4, 32'd13107);   // CONF[5]
        axi_write(32'h0300 + 5*4, 32'd4096);    // BOOST[5]
